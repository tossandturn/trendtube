...
export default function VideoAnalyzerBar() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Redirect to analyze results or video page
        ...
        <form onSubmit={handleAnalyze} className="flex items-center gap-3">
          <div className="flex-1 flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 bg-white border border-red-200 rounded-lg px-4 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
              disabled={loading}
            />
          </div>
          <button
